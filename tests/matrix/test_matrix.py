...
def test_matrix_column_based_positive(emptyApp, path):
...
def test_matrix_column_based_errors(emptyApp, config_json):
    plugin = EmptyPlugin(emptyApp)
...
def test_matrix_row_based_positive(emptyApp, path):
    plugin = EmptyPlugin(emptyApp)
    workbook_path = get_test_data_path(path)
